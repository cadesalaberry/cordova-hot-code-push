...
- (void)onNothingToInstallEvent:(NSNotification *)notification {
    ...
    // after installation reset options
    _installOptions = nil;
...
- (void)onUpdateInstalledEvent:(NSNotification *)notification {
    ...
    if (_installOptions && _installOptions[@"dontReload"]) {
        NSLog(@"Installed successfull without reloading!");
    } else {
...
- (void)jsInstallUpdate:(CDVInvokedUrlCommand *)command {
    ...
    // set install options, must be cleared after OnInstalledEvents
    _installOptions = command.arguments.count ? command.arguments[0] : nil;
